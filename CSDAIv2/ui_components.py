...
class AnalysisSession:
    ...
    def store_results(self, session_id: str, results) -> bool:
    ...
    def update_progress(self, session_id: str, stage: str, message: str, percentage: int = None) -> bool:
        """Update progress information"""
        if session_id in self.sessions:
            self.sessions[session_id].update({
                'analysis_stage': stage,
                'progress_message': message,
                'status': 'processing' if percentage and percentage < 100 else 'completed' if percentage == 100 else 'processing',
                'updated_at': datetime.now().isoformat()
            })
            if percentage is not None:
                self.sessions[session_id]['progress_percentage'] = percentage
            return True
        return False
    ...
    GUIDANCE = {
        'analysis_types': {
            ...
            'av_conflicts': {
                ...
                'file_requirements': ['RunningProcesses.xml'],
                'typical_use_cases': [
            ...
            'resource_analysis': {
                ...
                'file_requirements': ['RunningProcesses.xml', 'TopNBusyProcess.txt'],
                'typical_use_cases': [
                ...
                'preparation_tips': [
                    'Collect RunningProcesses.xml during normal system operation',
                ...
                'file_details': {
                    'RunningProcesses.xml': 'Complete list of all running processes and services',
